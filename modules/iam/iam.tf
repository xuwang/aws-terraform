...
variable "cloudinit_file_path" { default = "cloud-config/aws-files.yaml" }
variable "build_dir" { }
...
resource "aws_iam_user_policy" "deployment" {
    ...
    policy = "${file(\"${var.build_dir}/policies/deployment_policy.json\")}"
}
resource "aws_iam_access_key" "deployment" {
    ...
    command = <<CMD_DATA
cat > ${var.build_dir}/cloud-config/aws-files.yaml <<EOF
